...
get.mean = function(i.url, i.table)
{
  ...
  f.name = paste0("2. Data/",f.name)
  
  #write.csv(i.url,append=TRUE,file=f.name) # i.url
  s.url = sprintf("The url is %s",v.url[i.url])
  write.table(s.url,file=f.name, row.names=F,na="NA",append=T, quote= FALSE, sep=",", col.names=T)
  #write.csv(i.table,append=TRUE,file=f.name) # i.table
  write.table(i.table,file=f.name, row.names=F,na="NA",append=T, quote= FALSE, sep=",", col.names=T)
  write.csv(v.t,append=TRUE,file=f.name) # actual vector of values
...
g.1 = ggplot(data=df.winners) + theme_bw()

g.1 = g.1 + geom_line(data = filter(df.winners, grepl('COMEDY', genre)),
                      aes(x = year2, y = mean.us.viewers.m),alpha=0.1,size=0.5)
